...
mycmap = colormap(cmap); 
    ...
    if(h.lGray)        
        ...
        h.dImg = dImg;
        h.dPatch = dPatch;
        [h.hFront,h.hBack] = imoverlay(dImg(:,:,1,1),dPatch(:,:,1,1),h.colRange(1,:),h.colRange(2,:),mycmap,h.dAlpha, h.hI);
        %axes(h.hFront);
        colormap(cmap); 
    end
...
function fScroll(hObject, eventdata, handles)
    ...
    if(h.lGray)
        ...
        [h.hFront,h.hBack] = imoverlay(h.dImg(:,:,h.iActive,1),h.dPatch(:,:,h.iActive,1),h.colRange(1,:),h.colRange(2,:),jet(12),h.dAlpha, h.hI);
    end
